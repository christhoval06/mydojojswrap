...
define(["dojo", "dijit/registry", "dijit/form/Form", "modulos/form/field", "modulos/funciones/funciones", "dojo/NodeList-data"], function (dojo, registry, Form, Field, funciones) {
    ...
    function _createRow(row, tabla, callback, orden, evento, place) {
        Field.createRow(row, tabla, callback, orden, evento, place)
    }
        ...
        if (data.success) {
            ...
            if (data.form) {
                ...
                if (data.fields) {
                    ...
                    data.fields.orden.forEach(function (row) {
                        if (data.fields[row]) {
                            _createRow(data.fields[row], tabla, callback, data.orden.concat(otrosValores), evento, place);
                        }
                    });
